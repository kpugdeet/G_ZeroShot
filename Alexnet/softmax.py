...
class softmax (object):
    def __init__(self):
        ...
        with tf.variable_scope("softmax"):
            wF_H = tf.get_variable(name='wF_H', shape=[1000, globalV.FLAGS.numClass], dtype=tf.float32)
            bF_H = tf.get_variable(name='bF_H', shape=[globalV.FLAGS.numClass], dtype=tf.float32)
        ...
        # Loss
        self.totalLoss = tf.reduce_mean(tf.losses.softmax_cross_entropy(tf.one_hot(self.y, globalV.FLAGS.numClass), logits=self.yOut))
